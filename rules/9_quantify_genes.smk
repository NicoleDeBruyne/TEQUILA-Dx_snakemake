...
set per quantification method. See docs/rules/9_quantify_genes.md.

Four rules:
  1. _9A_plot_relative_gene_count    -- read-count-based proxy (number of
                                         distinct reads overlapping each
                                         gene's BED region), via
                                         scripts/quantify_gene_expression.py
                                         --metric count.
  2. _9B_plot_relative_gene_coverage -- max-coverage-based proxy (peak
                                         per-base pileup depth in each
                                         gene's BED region), via the same
                                         script with --metric coverage.
  3. _9D1-_9D6 (AMALGAM)             -- isoform discovery/quantification
                                         via AMALGAM (github.com/RNA-ROB/amalgam),
                                         run as its own dedicated 6-rule
                                         sub-pipeline: per-sample StringTie
                                         assembly (_9D1), group-level
                                         GffCompare merge against the
                                         reference annotation (_9D2),
                                         AMALGAM's own transcript filtering
                                         (_9D3) and (optional, currently
                                         unused downstream -- see _9D4's
                                         docstring) ORF annotation (_9D4),
                                         per-sample transcript quantification
                                         against the group's filtered
                                         transcriptome (_9D5), and a final
                                         cohort-wide matrix aggregation
                                         (_9D6). See setup.sh's AMALGAM
                                         section for how the tool itself and
                                         its dedicated conda env get
                                         installed.
  4. _9C_plot_relative_gene_by_assignment -- splice-site-sharing-based
...
_9A and _9B report two things per gene per sample: a raw value (read
...
on this BED panel) * 1e6. _9C reports the analogous raw assigned-read count
...
rule _9A_plot_relative_gene_count:
    input:
        bams = lambda wc: [SAMPLES[s]["bam"] for s in GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)]],
    ...
    params:
        names     = lambda wc: _quoted(GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)]),
        bams      = lambda wc: _quoted([SAMPLES[s]["bam"] for s in GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)]]),
    ...
    threads: lambda wc: _group_threads(_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type), "plot_relative_gene_count", config["threads"])
    resources:
...
rule _9B_plot_relative_gene_coverage:
    input:
        bams = lambda wc: [SAMPLES[s]["bam"] for s in GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)]],
        bed  = lambda wc: bed_path(wc.cohort_id, wc.bed_id),
    output:
    ...
    params:
        ...
        script    = workflow.basedir + "/scripts/quantify_gene_expression.py",
    threads: lambda wc: _group_threads(_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type), "plot_relative_gene_coverage", config["threads"])
    resources:
        mem_mb  = lambda wc, attempt: attempt * 1024 * max(8, len(GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)])),
        runtime = config["time"],
    log:
...
# ---------------------------------------------------------------------------
# AMALGAM sub-pipeline helpers (_9D1-_9D6). AMALGAM's own tool + assets +
...
def _amalgam_stringtie_gtf(cohort_id, bed_id, sample_type, sample):
    return _amalgam_group_dir(cohort_id, bed_id, sample_type) + "/stringtie/" + sample + "_stringtie.gtf"
...
rule _9D2_amalgam_merge_gtfs:
    ...
    input:
        ...
        sample_gtfs = lambda wc: [
            _amalgam_stringtie_gtf(wc.cohort_id, wc.bed_id, wc.sample_type, s)
...
rule _9D3_amalgam_build_transcriptome:
    ...
    # BOTH the uncompressed filtered.gtf (needed as-is by _9D4 below,
    # matching AMALGAM's own README) and the sorted/bgzip/tabix-indexed
    # filtered.gtf.gz (needed by _9D5) as real tracked outputs.
    input:
        combined_gtf = _cohort_outdir + "/{bed_id}/output/sample_types/{sample_type}/output/gene_quantification/by_amalgam/annotation/merged.combined.gtf",
        tracking      = _cohort_outdir + "/{bed_id}/output/sample_types/{sample_type}/output/gene_quantification/by_amalgam/annotation/merged.tracking",
        annotation    = config["annotation"],
...
rule _9D4_amalgam_annotate_orf:
    ...
    # downstream -- _9D5 (transcript quantification) runs against
    # _9D3's filtered.gtf.gz, not this rule's annotated.gtf.gz, matching
...
rule _9D5_amalgam_quantify_transcripts:
    # Step 5: Quantify_Transcripts.py, per sample, against the GROUP's
    # filtered transcriptome from _9D3 (every sample in a group shares
...
rule _9D6_amalgam_aggregate_matrices:
...
rule _9C_plot_relative_gene_by_assignment:
    input:
        bams = lambda wc: [SAMPLES[s]["bam"] for s in GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)]],
        bed  = lambda wc: bed_path(wc.cohort_id, wc.bed_id),
    ...
    threads: lambda wc: _group_threads(_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type), "plot_relative_gene_by_assignment", config["threads"])
    resources:
        ...
        # floor than _9A/_9B to account for that shared parsing cost.
        mem_mb  = lambda wc, attempt: attempt * 1024 * 2 * max(8, len(GROUPS[_group_id_from_ids(wc.cohort_id, wc.bed_id, wc.sample_type)])),
        runtime = config["time"],
    log:
